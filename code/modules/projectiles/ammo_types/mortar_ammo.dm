...
/datum/ammo/mortar/rocket/minelayer/drop_nade(turf/target_turf, atom/movable/projectile/proj)
	...
	if(proj && ismob(proj.firer))
		iff_signal = proj.firer.get_iff_signal()
		log_combat(proj.firer, mine, "deployed", src, "(IFF signal [iff_signal])")
	else
		if(usr)
			log_combat(usr, mine, "deployed", src, "(IFF signal [iff_signal])")
		else
