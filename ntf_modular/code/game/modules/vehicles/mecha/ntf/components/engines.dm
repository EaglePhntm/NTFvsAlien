...
/obj/item/mecha_parts/exosuit_engine
	...
	var/is_outputting_to_what = OUTPUT_MECH

/obj/item/cell/powertrain // idk what for, most mech stuff would be electric motor, arms maybe
	name = "powertrain"
	desc = "A 'cell' that holds physical motive energy generated by the engine. Should zero when the engine is off"
	maxcharge = 300
	charge_amount = 200 // idk
	charge = 0

/obj/item/cell/alternator // legs would be electric for stability and reaction speed?
	name = "alternator"
	desc = "A 'cell' that holds electrical energy generated by the engine. Should zero when the engine is off"
	maxcharge = 200
	charge_amount = 75
	charge = 0
...
/obj/item/mecha_parts/exosuit_engine/process()
	if(is_functional && is_running)
		var/current_consumption = fuel_consumption * (revs / 100)
		...
		if(engine_power_pool && !engine_power_pool.is_fully_charged())
			engine_power_pool.give(revs)

		if(starter_battery && !starter_battery.is_fully_charged())
			var/starter_charge_amount = (revs)
...
/obj/item/mecha_parts/exosuit_engine/proc/use_power(amount)
	...
	if(!engine_power_pool)
		return FALSE
	if(engine_power_pool.charge < amount)
		return FALSE
...
/obj/item/mecha_parts/exosuit_engine/proc/has_power(amount)
	...
	if(!engine_power_pool)
		return FALSE
	return engine_power_pool.charge >= amount
